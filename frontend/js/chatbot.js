...
class ChatbotManager {
    ...
    async sendMessage() {
        ...
        try {
            ...
            if (result.success && result.multiple && Array.isArray(result.data)) {
                console.log('📦 Multiple transactions detected:', result.data.length);
                
                let successCount = 0;
                for (const transData of result.data) {
                    const added = await this.addTransactionFromChatbot(transData);
                    if (added) successCount++;
                }
                
                if (successCount > 0) {
                    this.addBotMessage(`✅ Successfully added ${successCount} transaction(s)!\n${result.reply}`, 'success');
                } else {
                    this.addBotMessage(`⚠️ Received ${result.data.length} transaction(s) but failed to save.`, 'warning');
                }
            }
            // Handle single transaction
                ...
                // Add bot response
                if (transactionAdded) {
                    this.addBotMessage(`✅ ${result.reply || 'Transaction recorded!'}`, 'success');
                } else {
                    this.addBotMessage(result.reply || 'Transaction received but failed to save.', 'warning');
                }
            } else {
                // Add error message
                this.addBotMessage(result.error || 'Sorry, I couldn\'t understand that transaction. Try typing like: "Bought coffee $5" or "I paid bill $100 and got salary $150"', 'error');
            }
